							...
							char* dev_interface_path)
{
	...
	for(i = 0; i < priv->hid->nb_interfaces; i++) {
		if((priv->usb_interface[i].apib == &usb_api_backend[USB_API_HID]) &&
				(safe_strcmp(priv->usb_interface[i].path, dev_interface_path) == 0)) {
			// Avoid adding the same interface path twice
			return LIBUSB_SUCCESS;
		}
	}
